...
class EvidenceActivity : Activity() {
    ...
    private fun render() {
        ...
        val events = readRawEvents(state.rawTracePath)
        val overview = verticalPanel()
        overview.addView(label("事件概览"))
        if (events.isEmpty()) {
            overview.addView(body("还没有可读取的 raw eBPF 样例。请先运行“开始智能优化”，或者运行一次真实使用采集。"))
        ...
        feed.addView(label("可读事件样例"))
        if (events.isEmpty()) {
            feed.addView(body("没有可格式化的事件样例。"))
        } else {
            events.take(60).forEach { event -> feed.addView(eventCard(event)) }
        }
        root.addView(feed)
    }

    private fun readRawEvents(path: String): List<EBPFEvent> {
        if (path.isBlank()) return emptyList()
        val quoted = path.replace("'", "'\"'\"'")
        val result = RootShell.run("sed -n '/^MEMO/p' '$quoted' 2>/dev/null | head -n 160", timeoutMs = 8_000L)
        if (!result.ok || result.stdout.isBlank()) return emptyList()
        return EBPFTraceParser.parseLines(result.stdout.lineSequence())
    }
    ...
    private fun eventCard(event: EBPFEvent): View {
        ...
        panel.addView(text(eventName(event.eventType), 14f, Color.rgb(15, 23, 42), bold = true))
        ...
        panel.addView(text("时间: ${eventTime(event)}", 11f, Color.rgb(100, 116, 139)))
        return panel
    }
